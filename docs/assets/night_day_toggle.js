...
(function () {
  function setMode(mode) {
    if (mode === 'night') {
      document.documentElement.classList.add('night'); // changed
      localStorage.setItem('theme', 'night');
    } else {
      document.documentElement.classList.remove('night'); // changed
      localStorage.setItem('theme', 'day');
    }
  }
  ...
  if (btn) {
    btn.onclick = function () {
      var isNight = document.documentElement.classList.contains('night'); // changed
      setMode(isNight ? 'day' : 'night');
      btn.innerText = isNight ? '🌙 Night' : '☀️ Day';
    };
